  ...
  const relatedPosts = useMemo(() => {
    ...
    return blogPosts
      ...
      .map(candidate => {
        ...
        return {
          candidate,
          score: sharedTags * 3 + sameCategory * 2 + sameType,
        }
      })
      .sort((a, b) => {
  ...
  return (
    <main className="min-h-screen py-20">
      <div className="container mx-auto px-4 max-w-6xl">
              ...
              <div className="text-gray-300 space-y-4">
                {post.content.split('\n\n').map((paragraph, idx) => {
                  ...
                  if (paragraph.startsWith('# ')) {
                    ...
                    return <h1 id={headingIndex >= 0 ? toc[headingIndex].id : undefined} key={idx} className="text-2xl font-semibold text-white mt-8 mb-4 scroll-mt-24">{paragraph.slice(2)}</h1>
                  }
                  if (paragraph.startsWith('## ')) {
                    const headingIndex = toc.findIndex(item => item.text === paragraph.slice(3))
                    return <h2 id={headingIndex >= 0 ? toc[headingIndex].id : undefined} key={idx} className="text-xl font-semibold text-white mt-6 mb-3 scroll-mt-24">{paragraph.slice(3)}</h2>
                  }
                  if (paragraph.startsWith('### ')) {
                    const headingIndex = toc.findIndex(item => item.text === paragraph.slice(4))
                    return <h3 id={headingIndex >= 0 ? toc[headingIndex].id : undefined} key={idx} className="text-lg font-semibold text-white mt-4 mb-2 scroll-mt-24">{paragraph.slice(4)}</h3>
                  }
        ...
        {/* Comments Section */}
        <motion.div variants={itemVariants} className="mt-16 pt-8 border-t border-gray-700 max-w-3xl">
